...
function UI.createMainGUI()
    ...
    local function createSliderRow(name, labelText, minVal, maxVal, defaultVal, order, callback)
        local rowFrame = Instance.new("Frame")
        rowFrame.Name = name
    ...
    createCycleRow("TargetMode", "Target Mode", 3, {"distance", "health"}, function(selected)
        Config.TargetingMode = selected
    end)

    createSliderRow("DistanceSlider", "Distance", 100, 1000, ESPConfig.MaxDistance, 4, function(val)
