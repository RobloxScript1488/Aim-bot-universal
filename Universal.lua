-- Universal Script with Rayfield Library & Fixes
...
--------------------------------------------------------------------------------
-- STATS COUNTERS (FPS & PING)
--------------------------------------------------------------------------------
CombatTab:CreateSection("Performance Stats")
local FPSLabel = CombatTab:CreateLabel("FPS: 0")
local PingLabel = CombatTab:CreateLabel("Ping: 0 ms")

--------------------------------------------------------------------------------
-- MODULE 1: CHINA HAT & KILL SOUND
--------------------------------------------------------------------------------
CombatTab:CreateSection("Visuals & Audio")
local HatPart = nil
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if now - lastFpsUpdate >= 1 then
        ...
        if success and pingVal then
            PingLabel:Set("Ping: " .. tostring(pingVal) .. " ms")
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    FOVFrame.Visible = Config.Aimbot.ShowFOV
...
Rayfield:Notify({ Title = "Script Updated", Content = "FPS, Ping & FOV fix applied successfully!", Duration = 4 })
